...
class PatchComputerGpu {
    ...
    private constructor(device: GPUDevice, localCacheSize: THREE.Vector3, vertexDataEncoder: VertexDataEncoder) {
        this.device = device;
        ...
        fn encodeVoxelData(localPositionX: u32, localPositionY: u32, localPositionZ: u32, voxelMaterialId: u32) -> u32 {
            return ${vertexDataEncoder.wgslEncodeVoxelData('localPositionX', 'localPositionY', 'localPositionZ', 'voxelMaterialId')};
        }
        fn encodeVertexData(voxelData: u32, ao: u32, edgeRoundnessX: u32, edgeRoundnessY: u32) -> u32 {
            return voxelData + ${vertexDataEncoder.wgslEncodeVertexData('ao', 'edgeRoundnessX', 'edgeRoundnessY')};
        }
        ...
        fn main(in: ComputeIn) {
            ...
            if (voxelLocalPosition.z < patchSize.z) { // if we are in the patch
                ...
                if (voxelData != 0u) {
                    ...
                    let voxelData = encodeVoxelData(voxelLocalPosition.x, voxelLocalPosition.y, voxelLocalPosition.z,
                        voxelMaterialId
                    );
                    ${Object.values(Cube.faces)
                        .map(
                    ...
                    if (!doesNeighbourExist(cacheIndex, vec3i(${face.normal.vec.x}, ${face.normal.vec.y}, ${face.normal.vec.z}))) {
                        ...
                        ${face.type}FaceVerticesData.verticesData[firstVertexIndex + ${index}u] = vertex${faceVertexId}Data;`
                            )
                            .join('')}
                    }`
                        )
