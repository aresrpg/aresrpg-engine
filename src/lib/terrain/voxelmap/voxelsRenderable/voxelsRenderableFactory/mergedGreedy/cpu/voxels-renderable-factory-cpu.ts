...
class VoxelsRenderableFactoryCpu extends VoxelsRenderableFactory {
    private readonly serializableFactory = {
        ...
        buildBuffer(voxelsChunkData: VoxelsChunkData): Uint32Array {
            ...
            const registerFace = (faceData: FaceData) => {
            ...
            for (localPosition.z = 1; localPosition.z < voxelsChunkCache.size.z - 1; localPosition.z++) {
                for (localPosition.y = 1; localPosition.y < voxelsChunkCache.size.y - 1; localPosition.y++) {
                    for (localPosition.x = 1; localPosition.x < voxelsChunkCache.size.x - 1; localPosition.x++) {
                        const cacheIndex = voxelsChunkCache.buildIndex(localPosition);
                        ...
                        for (const face of Object.values(this.cube.faces)) {
                            ...
                            const faceData = {
                                ...
                                faceType: face.type,
                                faceId: face.id,
                                faceNoiseId: faceId++ % this.vertexData2Encoder.faceNoiseId.maxValue,
